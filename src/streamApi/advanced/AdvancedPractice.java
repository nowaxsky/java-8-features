...
public class AdvancedPractice {
  ...
  public static void main(String[] args) {
    ...
    // 依性別做分類
    Map<Gender, List<User>> genderGroupBy = users.stream().collect(Collectors.groupingBy(User::getGender));
    genderGroupBy.forEach((g, u) -> System.out.println(g + ":\t" + u.stream().map(User::getName).collect(Collectors.joining(", "))));

    // 依是否為女生來做分類
    Map<Boolean, List<User>> genderPartitionBy = users.stream().collect(Collectors.partitioningBy(u -> u.getGender() == Gender.F));
    genderPartitionBy.forEach((g, u) -> System.out.println(g + ":\t" + u.stream().map(User::getName).collect(Collectors.joining(", "))));
    
    // 依性別來做分類並以UserBrief物件蒐集
    List<UserBrief> collect = users.stream().collect(Collectors.mapping(u -> new UserBrief(u.getName(), u.getAge()), Collectors.toList()));
